...
public class EmployeeProfile extends BaseModuleScreen implements ModuleScreen {
    ...
    private static void showPayrollEditDialog(JFrame parentFrame, Employee employee) {
        ...
        if (record == null || record.getHourlyRate() <= 0) {
        ...
        double estimatedMonthlyGross = record.getHourlyRate() * 160.0;
        ...
        saveButton.addActionListener(e -> {
            try {
                ...
                double estimatedGross = hourlyRate * 160.0;
                double sssAmount = PayrollUtils.calculateSSSAmount(estimatedGross);
                double philHealthAmount = PayrollUtils.calculatePhilHealthAmount(estimatedGross);
                double pagIbigAmount = PayrollUtils.calculatePagIbigAmount(estimatedGross);
                ...
                double withHoldingTax = PayrollUtils.calculateWithholdingTax(estimatedGross, riceSubsidy, phoneAllowance, clothingAllowance);

                PayrollData newRecord = new PayrollData(
                    estimatedGross, hourlyRate, sssAmount, philHealthAmount, pagIbigAmount, (float)withHoldingTax,
    ...
    private static void updateSummaryArea(JTextArea summaryArea, PayrollData record) {
        ...
        double estimatedGross = record.getHourlyRate() * 160.0;
        summary.append("Estimated Gross (160h): ₱").append(String.format("%.2f", estimatedGross)).append("\n");
        ...
        summary.append("Estimated Net (160h): ₱").append(String.format("%.2f", record.calculateNetSalaryForHours(160.0)));
    ...
    private static void initializePayrollRecord(String employeeId, String position, double baseSalary) {
        ...
        if (existing == null || existing.getHourlyRate() <= 0) {
            ...
            PayrollData record = new PayrollData(baseSalary, hourlyRate, sssAmount, philHealthAmount, pagIbigAmount, (float) tax, riceSubsidy, phoneAllowance, clothingAllowance);
    ...
    private static void showNewEmployeeForm(JFrame parentFrame) {
        ...
        addFormField(mainPanel, "Hourly Rate:", salaryField, gbc, 12);
        addFormField(mainPanel, "Rice Subsidy:", riceSubsidyField, gbc, 13);
        addFormField(mainPanel, "Phone Allowance:", phoneAllowanceField, gbc, 14);
        addFormField(mainPanel, "Clothing Allowance:", clothingAllowanceField, gbc, 15);
        ...
        submitButton.addActionListener(e -> {
            try {
                ...
                double hourlyRate = Double.parseDouble(salaryField.getText());
                double baseSalary = hourlyRate * 168.0;
