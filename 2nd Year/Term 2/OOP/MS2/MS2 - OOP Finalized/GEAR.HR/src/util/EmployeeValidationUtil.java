...
public final class EmployeeValidationUtil {
    ...
    private static final Pattern SSS = Pattern.compile("^[0-9]{9}$");
    private static final Pattern PHIL_HEALTH = Pattern.compile("^[0-9]{12}$");
    private static final Pattern TIN = Pattern.compile("^[0-9]{3}-[0-9]{3}-[0-9]{3}$");
    ...
    public static String validateSss(String value) {
        String s = value != null ? value.trim() : "";
        if (s.isEmpty()) return "SSS is required. Must be exactly 9 digits (e.g. 123456789).";
        if (!SSS.matcher(s).matches()) return "SSS must be exactly 9 digits (e.g. 123456789).";
        return null;
    }
    ...
    public static String validateTin(String value) {
        String s = value != null ? value.trim() : "";
        if (s.isEmpty()) return "TIN is required. Must be 9 digits in format 000-000-000 (e.g. 123-456-789).";
        if (!TIN.matcher(s).matches()) return "TIN must be 9 digits in format 000-000-000 (e.g. 123-456-789).";
        return null;
    }
