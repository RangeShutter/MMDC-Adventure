    ...
    private static JPanel createSidebarPanel(JFrame mainFrame, String userId, String role, String email, RoleGroup group, ApplicationContext ctx) {
        ...
        switch (group) {
            case HR:
                ...
                sidebar.add(Box.createVerticalStrut(8));
                sidebar.add(createSidebarButton("Leave Management", e ->
                    LeaveManagementScreen.showLeaveScreen(mainFrame, userId, role, group, ctx.getLeaveService(), ctx.getEmployeeService())));
                break;
            case PAYROLL:
                sidebar.add(createSidebarButton("Payroll Management", e ->
                    EmployeeProfile.showProfileScreen(mainFrame, userId, role, group, ctx.getEmployeeService(), ctx.getAttendanceService(), ctx.getPayrollProcessor())));
                sidebar.add(Box.createVerticalStrut(8));
                sidebar.add(createSidebarButton("View Attendance", e ->
                    AttendanceScreen.showAttendanceScreen(mainFrame, userId, role, group, ctx.getAttendanceService(), ctx.getEmployeeService())));
                sidebar.add(Box.createVerticalStrut(8));
                sidebar.add(createSidebarButton("View Leave Requests", e ->
                    LeaveManagementScreen.showLeaveScreen(mainFrame, userId, role, group, ctx.getLeaveService(), ctx.getEmployeeService())));
                break;
            default: // NORMAL
