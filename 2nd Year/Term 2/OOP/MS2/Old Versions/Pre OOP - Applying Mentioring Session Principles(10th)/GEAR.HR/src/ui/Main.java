...
public class Main {
        ...
        switch (group) {
            ...
            default: // NORMAL
                sidebar.add(createSidebarButton("My Attendance", e ->
                    AttendanceScreen.showAttendanceScreen(mainFrame, userId, role, group, ctx.getAttendanceService(), ctx.getEmployeeService())));
                sidebar.add(Box.createVerticalStrut(8));
                sidebar.add(createSidebarButton("My Profile", e ->
                    EmployeeProfile.showProfileScreen(mainFrame, userId, role, group, ctx.getEmployeeService(), ctx.getAttendanceService(), ctx.getPayrollProcessor())));
                sidebar.add(Box.createVerticalStrut(8));
                sidebar.add(createSidebarButton("My Payroll", e ->
                    EmployeeProfile.showProfileScreen(mainFrame, userId, role, group, ctx.getEmployeeService(), ctx.getAttendanceService(), ctx.getPayrollProcessor())));
                sidebar.add(Box.createVerticalStrut(8));
                sidebar.add(createSidebarButton("My Leave", e ->
                    LeaveManagementScreen.showLeaveScreen(mainFrame, userId, role, group, ctx.getLeaveService(), ctx.getEmployeeService())));
                break;
        }
    ...
    private static JPanel createPersonalAccountCollapsible(JFrame mainFrame, String userId, String role, ApplicationContext ctx) {
        ...
        content.add(createSidebarButton("My Attendance", e ->
            AttendanceScreen.showAttendanceScreen(mainFrame, userId, role, RoleGroup.NORMAL, ctx.getAttendanceService(), ctx.getEmployeeService())));
        content.add(Box.createVerticalStrut(6));
        content.add(createSidebarButton("My Profile", e ->
            EmployeeProfile.showProfileScreen(mainFrame, userId, role, RoleGroup.NORMAL, ctx.getEmployeeService(), ctx.getAttendanceService(), ctx.getPayrollProcessor())));
        content.add(Box.createVerticalStrut(6));
        content.add(createSidebarButton("My Payroll", e ->
            EmployeeProfile.showProfileScreen(mainFrame, userId, role, RoleGroup.NORMAL, ctx.getEmployeeService(), ctx.getAttendanceService(), ctx.getPayrollProcessor())));
        content.add(Box.createVerticalStrut(6));
        content.add(createSidebarButton("My Leave", e ->
            LeaveManagementScreen.showLeaveScreen(mainFrame, userId, role, RoleGroup.NORMAL, ctx.getLeaveService(), ctx.getEmployeeService())));
