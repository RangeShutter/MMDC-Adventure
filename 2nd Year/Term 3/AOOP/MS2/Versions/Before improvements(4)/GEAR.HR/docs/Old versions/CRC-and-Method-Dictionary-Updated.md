# GEAR.HR — CRC Cards and Method Dictionary (Updated)

GEAR.HR is a desktop HR system built in **Java Swing** for managing employees, attendance, leave requests, and payroll. This document describes our **target design** after persistence and payroll expansion: primary storage is **MySQL via JDBC**, with CSV repositories retained as legacy fallback during migration.

**Sources:** [Payroll System Expansion Planning.md](Payroll%20System%20Expansion%20Planning.md), [Persistence Design Revision Log.md](Persistence%20Design%20Revision%20Log.md), [HM1.md](HM1.md), [HM3.md](HM3.md), [HM4.md](HM4.md).

**Baseline (current code):** [CRC-and-Method-Dictionary.md](CRC-and-Method-Dictionary.md) — CSV-only. **Related:** [Architecture-Overview-Updated.md](Architecture-Overview-Updated.md).

The application entry point is `ui.Main`. UI modules have **no** persistence responsibilities; they call services through `ApplicationContext` only.
...
| `PayPeriod` | **(Planned)** Domain model for a payroll month: month, year, status (draft, reviewed, finalized), finalizedBy, finalizedAt. Validates status transitions. Implements Validatable. One closed book per calendar month. |
| `PayrollRunSnapshot` | **(Planned)** Frozen per-employee pay result for a pay period: periodId, employeeId, gross, deductions, allowances, net, hoursUsed. Historical reports use snapshots so live rate changes do not rewrite closed months. |
| `AuditLogEntry` | **(Planned)** Append-only audit record: userId, timestamp, optional targetEmployeeId, actionType (e.g. UPDATE_PAYROLL, FINALIZE_PERIOD). Supports HM3 accountability minimum. |
| `DatabaseConnectionManager` | **(Planned)** Opens and closes JDBC connections to MySQL; supplies connections to repository classes. Centralizes connection configuration so repositories do not duplicate JDBC boilerplate. |
| `AbstractJdbcRepository` | **(Planned)** Abstract template for JDBC repositories: shared query execution and row-mapping helpers; subclasses provide entity-specific SQL. Collaborator: DatabaseConnectionManager. Mirrors AbstractCsvListRepository pattern. |
| `AbstractCsvListRepository` | Abstract base for CSV repositories that persist a list of entities. **(Legacy — CSV fallback during migration.)** Subclasses provide file path, header, and line parsing/serialization; this class implements the common load/save file I/O. Concrete repos extend and implement the abstract hooks. |
| `EmployeeJdbcRepository` | **(Planned)** JDBC persistence for employees via MySQL. Implements IEmployeeRepository; load/save only. Collaborators: DatabaseConnectionManager, Employee. |
| `AttendanceJdbcRepository` | **(Planned)** JDBC persistence for attendance records. Implements IAttendanceRepository. Collaborators: DatabaseConnectionManager, AttendanceRecord. |
| `LeaveRequestJdbcRepository` | **(Planned)** JDBC persistence for leave requests. Implements ILeaveRequestRepository. Collaborators: DatabaseConnectionManager, LeaveRequest. |
| `UserCredentialJdbcRepository` | **(Planned)** JDBC persistence for user credentials. Implements IUserCredentialRepository. Collaborators: DatabaseConnectionManager, UserCredential. |
| `ItTicketJdbcRepository` | **(Planned)** JDBC persistence for IT tickets. Implements IItTicketRepository. Collaborators: DatabaseConnectionManager, ItTicket. |
| `PayrollJdbcRepository` | **(Planned)** JDBC persistence for payroll settings (employeeId → PayrollData). Implements IPayrollRepository. Collaborators: DatabaseConnectionManager, PayrollData. |
| `IPayPeriodRepository` | **(Planned)** Contract for pay period persistence: findById, save, listByStatus. Implementations swappable (JDBC). |
| `PayPeriodJdbcRepository` | **(Planned)** JDBC implementation of IPayPeriodRepository. Collaborators: DatabaseConnectionManager, PayPeriod. |
| `IPayrollRunRepository` | **(Planned)** Contract: saveAllForPeriod(periodId, snapshots), findByPeriod(periodId), findByPeriodAndEmployee. |
| `PayrollRunJdbcRepository` | **(Planned)** JDBC implementation of IPayrollRunRepository. Collaborators: DatabaseConnectionManager, PayrollRunSnapshot. |
| `IAuditLogRepository` | **(Planned)** Contract: append(entry); findByEmployee; findByPeriod for oversight reads. |
| `AuditLogJdbcRepository` | **(Planned)** JDBC implementation of IAuditLogRepository. Append-only audit trail. Collaborators: DatabaseConnectionManager, AuditLogEntry. |
| `AttendanceRepository` | CSV persistence for attendance records. Load/save only; no business logic. **(Legacy — CSV fallback.)** Implements IAttendanceRepository. Extends AbstractCsvListRepository for shared CSV load/save; provides AttendanceRecord-specific parse/serialize. Can be used as IAttendanceRepository by callers. |
| `EmployeeRepository` | CSV persistence for employees (employees.csv). Load/save only; no business logic. **(Legacy — CSV fallback.)** Implements IEmployeeRepository. Extends AbstractCsvListRepository for shared CSV load/save; provides Employee-specific parse/serialize. Can be used as IEmployeeRepository by callers. |
...
| `ItTicketRepository` | CSV persistence for IT support tickets. Load/save only; no business logic. **(Legacy — CSV fallback.)** Implements IItTicketRepository. Extends AbstractCsvListRepository for shared CSV load/save; provides ItTicket-specific parse/serialize. Can be used as IItTicketRepository by callers. |
...
| `PayrollRepository` | CSV persistence for payroll data (employeeId -> PayrollData). Load/save only. **(Legacy — CSV fallback.)** Expects map values to already include withholding tax when saving. Implements IPayrollRepository. Can be used as IPayrollRepository by callers. |
| `UserCredentialRepository` | CSV persistence for user credentials. Load/save only; no business logic. **(Legacy — CSV fallback.)** Implements IUserCredentialRepository. Extends AbstractCsvListRepository for shared CSV load/save; provides UserCredential-specific parse/serialize. Can be used as IUserCredentialRepository by callers. |
| `ApplicationContext` | Composition root: wires **JDBC** repository implementations for MySQL at startup (target); optional flag/factory to use CSV repos during migration. Creates services; passed into UI (DI). Stores and exposes interface types so callers depend on abstractions. Collaborators: all I*Repository interfaces, *JdbcRepository classes, services. |
...
| `PayrollProcessor` | Computes employee compensation; orchestrates payroll persistence and HM3 use cases. **(Planned additions:** batch run for all employees per period, load snapshots, finalize period, append audit after successful saves.) Depends on IPayrollRepository, IAttendanceService, IPayPeriodRepository, IPayrollRunRepository, IAuditLogRepository. |
| `PayrollReport` | Formats payroll results for display. **(Planned additions:** payroll register and deduction summary from PayrollRunSnapshot lists.) Separates presentation text from PayrollProcessor computation. |
