# GEAR.HR — Architectural Overview (Updated v2)
...
**Version:** v2 adds UML class and use case diagrams, expanded JDBC vs ORM **design discussion**, and richer open questions with context.

**Baseline (current code):** [Architecture-Overview.md](Architecture-Overview.md) — CSV-only. **Prior target:** [Architecture-Overview-Updated.md](Architecture-Overview-Updated.md). **Design artifacts:** [CRC-and-Method-Dictionary-Updated-v2.md](CRC-and-Method-Dictionary-Updated-v2.md). **Planning:** [Payroll System Expansion Planning-v2.md](Payroll%20System%20Expansion%20Planning-v2.md), [Persistence Design Revision Log-v2.md](Persistence%20Design%20Revision%20Log-v2.md), [HM3.md](HM3.md), [HM4.md](HM4.md).
...
**Design discussion — why JDBC here:** GEAR.HR already separates persistence behind repository interfaces and a composition-root swap (`ApplicationContext`). JDBC extends that pattern with `*JdbcRepository` classes rather than introducing an ORM entity manager that would cut across domain and mapping concerns. For payroll finalize, we need visible multi-table transactions (snapshots + period status + audit); JDBC keeps that orchestration explicit in `PayrollProcessor` and repository code (see §6a).
...
## 6a. Persistence design discussion — JDBC vs ORM

This section compares persistence approaches at a **structural** level for GEAR.HR. We are not claiming JDBC is universally superior to ORM; we explain why JDBC fits *this* layered desktop design and coursework path better than an ORM-first approach.

| Dimension | JDBC (our direction) | ORM (conceptual alternative) |
|-----------|----------------------|--------------------------------|
| **Responsibility placement** | SQL and row mapping live in `repository` (`AbstractJdbcRepository`, `*JdbcRepository`); services call `I*Repository` only | Mapping annotations or entity manager often blur domain and persistence; services may depend on generated queries or lazy-loaded associations |
| **Visibility in design** | Persistence is explicit in CRC cards, class diagrams, and sequence diagrams (`Database`, `INSERT`, transactions) | SQL and fetch strategy hidden; harder to trace finalize/batch behavior in design artifacts |
| **Fit with existing GEAR.HR** | Mirrors CSV repository swap: new JDBC classes implement same `I*Repository` contracts; `ApplicationContext` changes wiring only | Would likely refactor `PayrollData` (not `AbstractEntity`, keyed by map), HM3 snapshot/audit tables, and load-all service patterns |
| **Transactions (batch / finalize)** | `PayrollProcessor.finalizePeriod` can coordinate `saveAllForPeriod`, period update, and audit append in one JDBC transaction | ORM can manage transactions, but behavior is framework-dependent and less visible for mentor review |
| **HM3 audit and snapshots** | Append-only `audit_log` and bulk snapshot inserts are straightforward SQL | Requires careful mapping (immutable entities, cascade rules) to avoid accidental updates |
| **Course alignment** | JDBC integration is the stated course path for database connection | Adds framework learning curve (JPA/Hibernate config) beyond project scope |
| **Drawbacks** | More boilerplate SQL; manual schema evolution; services may keep CSV-era load-all patterns until refactored | Less control over queries; N+1 risk; harder to model non-entity `PayrollData` without workarounds |

**Design discussion — conclusion:** We chose JDBC because it preserves the dependency rule (`ui → service → repository → model`), minimizes churn from our CSV repository habit, and makes payroll office flows (batch run, finalize, audit) **auditable in design and code**. An ORM could reduce CRUD lines for simple tables, but GEAR.HR’s sensitive payroll workflows and existing interface-based structure benefit more from explicit JDBC repositories than from ORM abstraction at this milestone.
...
Actors and use cases below map to role groups in §9 and sequence flows in §11.
...
flowchart TB
  ...
  subgraph hrOps [HR Operations]
    UC_Att[Manage Attendance]
    UC_Emp[Manage Employees]
    UC_Leave[Manage Leave Requests]
    UC_ViewPayReadOnly[View Payroll Read Only]
  end

  subgraph payrollOffice [Payroll Office]
    UC_PaySettings[Maintain Pay Settings]
    UC_BatchRun[Run Batch Payroll]
    UC_Finalize[Finalize Pay Period]
    UC_Reports[Generate Payroll Reports]
    UC_ViewOwnPay[View Own Pay Slip]
  end

  subgraph admin [Administration]
    UC_Creds[Manage User Credentials]
    UC_Tickets[Manage IT Tickets]
  end
  ...
  HR --> UC_Emp
  HR --> UC_Leave
  HR --> UC_ViewPayReadOnly
  PayStaff --> UC_Login
  PayStaff --> UC_PaySettings
  PayStaff --> UC_BatchRun
  PayStaff --> UC_Finalize
  PayStaff --> UC_Reports
  ITAdmin --> UC_Login
  ITAdmin --> UC_Att
  ITAdmin --> UC_Emp
  ITAdmin --> UC_PaySettings
  ITAdmin --> UC_Creds
  ITAdmin --> UC_Tickets
  UC_Login -.->|include| UC_Auth
  Sys --> UC_Auth
  UC_BatchRun -.->|include| UC_Reports
  UC_Finalize -.->|include| UC_ViewOwnPay
```

**Design discussion:** Employees interact only with personal modules; HR manages people and time but not pay edits; payroll staff own month-end (`runPayrollForPeriod`, `finalizePeriod`); IT/admin retains broad access for support. System-level authentication and statutory computation sit behind services (`AuthenticationService`, `PayrollProcessor`, `PayrollUtils`).

### 7b. UML class diagram — domain model
...
classDiagram
  ...
  PayPeriod "1" --> "many" PayrollRunSnapshot : periodId
  PayrollRunSnapshot ..> Employee : employeeId
```

**Design discussion:** Domain types do not reference JDBC or SQL. `PayrollData` remains a value object keyed by employee id in storage (not extending `AbstractEntity`). HM3 types (`PayPeriod`, `PayrollRunSnapshot`, `AuditLogEntry`) support workflow and history without changing employee/attendance models.

### 7c. UML class diagram — persistence layer
...
classDiagram
  ...
  class IPayPeriodRepository {
    <<interface>>
    +findById(id) PayPeriod
    +save(period) void
  }
  class IPayrollRunRepository {
    <<interface>>
    +saveAllForPeriod(id, snapshots) void
    +findByPeriod(id) List~PayrollRunSnapshot~
  }
  ...
  class AbstractCsvListRepository {
    <<abstract legacy>>
  }
  ...
  AbstractJdbcRepository <|-- PayrollJdbcRepository
  ...
  AbstractCsvListRepository <|-- EmployeeRepository
  EmployeeJdbcRepository --> DatabaseConnectionManager
  PayrollJdbcRepository --> DatabaseConnectionManager
  PayPeriodJdbcRepository --> DatabaseConnectionManager
  PayrollRunJdbcRepository --> DatabaseConnectionManager
  AuditLogJdbcRepository --> DatabaseConnectionManager
```

**Design discussion:** Repositories implement interfaces only; JDBC subclasses share connection and query helpers via `AbstractJdbcRepository`. CSV classes remain as optional legacy implementations of the same interfaces—swap occurs in `ApplicationContext`, not in services.

### 7d. UML class diagram — application and presentation layers
...
classDiagram
  ...
  class EmployeeService {
    +updateEmployee(emp) String
  }
  class PayrollProcessor {
    +getPayrollForEmployee(id, periodId) PayrollResult
    +runPayrollForPeriod(periodId) List~PayrollRunSnapshot~
    +finalizePeriod(periodId, userId) void
  }
  class PayrollReport {
    <<utility>>
    +format(result) String
    +formatPayrollRegister(snapshots) String
  }
  ...
  ApplicationContext --> EmployeeService
  ApplicationContext --> PayrollProcessor
  EmployeeProfile --> ApplicationContext
  EmployeeService --> IEmployeeRepository
  PayrollProcessor --> IPayrollRepository
  ...
  PayrollReport ..> PayrollRunSnapshot : formats
  PayrollProcessor ..> PayrollReport : delegates formatting
```

**Design discussion:** UI depends on `ApplicationContext` only; services orchestrate use cases and delegate persistence to `I*Repository`. `PayrollProcessor` is the payroll use-case hub—batch run, finalize, and audit append stay out of `EmployeeProfile`. This layering is why JDBC fits better than ORM here: we add JDBC repo classes without annotating domain entities or changing UI contracts.
...
| **Repository swap** | `ApplicationContext` wires JDBC or CSV without changing services/UI |
| **Transaction boundary** | Explicit JDBC transaction on batch save + finalize (design discussion; provisional scope) |
...
These interaction diagrams implement the use cases in §7a (employee update, payroll view, batch finalize, audit on pay edit).
...
**Status:** This document and [CRC-and-Method-Dictionary-Updated-v2.md](CRC-and-Method-Dictionary-Updated-v2.md) describe the **target** design (v2). The running app in `src` still uses CSV repositories per [Architecture-Overview.md](Architecture-Overview.md). Prior version: [Architecture-Overview-Updated.md](Architecture-Overview-Updated.md).

**Open questions for implementation** (with context):

1. **Schema keys (business vs surrogate IDs)** — *Why it matters:* Snapshots and audit rows reference employees and periods for years; employee numbers match CSV today. *Blocks:* DDL, foreign keys, import script. *Options:* `employee_number` as primary key vs surrogate `id` plus unique business key.

2. **CSV import vs manual re-entry** — *Why it matters:* ~38 employees and six file types must land in MySQL consistently. *Blocks:* Phase 1 go-live. *Options:* one-time import tool vs SQL seed vs hybrid (employees first, then payroll rows).

3. **Export format for payroll register** — *Why it matters:* HM3 reports must be usable by payroll office and leadership. *Blocks:* Phase 3 UI/report deliverable. *Options:* print dialog, CSV export, PDF (later milestone).

4. **Period status and HR “reviewed”** — *Why it matters:* Workflow affects who can finalize and what HR sees before close. *Blocks:* `PayPeriod` state machine and UI labels. *Options:* informational “reviewed” vs mandatory approval step before finalize.

5. **Finalized period edit rules** — *Why it matters:* Immutability of closed months vs operational fixes. *Blocks:* whether live `PayrollData` edits are blocked when a period is finalized. *Options:* snapshots-only immutability vs lock live settings vs override role for IT.

6. **Transaction scope** — *Why it matters:* Partial finalize (snapshots saved but period not updated) corrupts month-end. *Blocks:* JDBC connection handling in `PayrollProcessor`. *Options:* transactions only on `saveAllForPeriod`/`finalizePeriod` vs every repository call.

7. **Field-level audit vs action-type v1** — *Why it matters:* Compliance vs implementation cost. *Blocks:* `AuditLogEntry` schema and `PayrollProcessor` hooks. *Options:* action-type only (HM3 v1) vs diff columns later.

8. **Payroll office UI placement** — *Why it matters:* Batch run and finalize need clear UX for payroll staff. *Blocks:* navigation in `Main` / `EmployeeProfile`. *Options:* new `ModuleScreen` vs extra tabs on `EmployeeProfile`.

9. **MySQL deployment and credentials** — *Why it matters:* Every JDBC repo depends on connection config. *Blocks:* local dev and demo setup. *Options:* local MySQL instance vs shared course server; credentials via env/config outside repo.

10. **ORM revisit trigger** — *Design discussion:* If entity count and CRUD complexity grow beyond coursework scope, we could introduce ORM for simple tables while keeping JDBC for payroll finalize paths—but that split adds dual persistence styles and is not planned for current milestones.
