# GEAR.HR — CRC Cards and Method Dictionary (Updated v2)

GEAR.HR is a desktop HR system built in **Java Swing**. This document describes our **target design** after persistence and payroll expansion: primary storage is **MySQL via JDBC**, with CSV repositories retained as legacy fallback during migration.

**Version:** v2 adds standard CRC **Collaborators** column, explicit responsibility delegation, and interaction highlights.

**Sources:** [Payroll System Expansion Planning-v2.md](Payroll%20System%20Expansion%20Planning-v2.md), [Persistence Design Revision Log-v2.md](Persistence%20Design%20Revision%20Log-v2.md), [HM1.md](HM1.md), [HM3.md](HM3.md), [HM4.md](HM4.md).

**Baseline:** [CRC-and-Method-Dictionary.md](CRC-and-Method-Dictionary.md) (CSV). **Prior target:** [CRC-and-Method-Dictionary-Updated.md](CRC-and-Method-Dictionary-Updated.md). **Related:** [Architecture-Overview-Updated-v2.md](Architecture-Overview-Updated-v2.md).

**Design discussion:** CRC cards document how responsibilities are delegated across layers. UI modules call services only; services orchestrate use cases and call `I*Repository`; repositories perform load/save via JDBC (or CSV fallback). Audit entries are appended by `PayrollProcessor` after successful persistence—not by UI or repositories alone.

### Interaction highlights (delegation chains)

1. **Employee update:** `EmployeeProfile` → `EmployeeService` → `EmployeeValidationUtil` → `IEmployeeRepository` → `EmployeeJdbcRepository` → `DatabaseConnectionManager`
2. **Payroll view (finalized):** `EmployeeProfile` → `PayrollProcessor` → `IPayrollRunRepository` → `PayrollRunJdbcRepository` → MySQL snapshots
3. **Batch finalize:** `EmployeeProfile` → `PayrollProcessor` → `IPayrollRunRepository` + `IPayPeriodRepository` + `IAuditLogRepository` (coordinated transaction)
4. **Login:** `User` → `AuthenticationService` → `IUserCredentialRepository` → `UserCredentialJdbcRepository`
5. **Pay edit audit:** `PayrollProcessor` → `IPayrollRepository.save` then `IAuditLogRepository.append`

The application entry point is `ui.Main`.

## Table 1 — CRC Cards

| Class Name | Responsibilities | Collaborators |
|------------|------------------|---------------|
| `AbstractEntity` | Abstract base class that defines shared characteristics of domain entities that have an identifier. Subclasses (Employee, AttendanceRecord, LeaveRequest, PayrollResult) extend this class and provide their own validation logic. Implements Identifiable and Validatable. | Identifiable, Validatable |
| `AttendanceRecord` | Domain model for a single attendance entry (OOP redesign - GEAR.HR). Child class extends AbstractEntity. Validates time-out after time-in and non-negative hours worked. | AbstractEntity, Employee (by employeeId) |
| `Employee` | Domain model representing an employee (OOP redesign - GEAR.HR). Child class extends AbstractEntity. Encapsulates personal, work, and payroll-related attributes. Validation in setters per OOP.txt: non-negative rates, non-empty password, valid roles. Format validation delegated to EmployeeValidationUtil for consistency at persistence boundaries. | AbstractEntity, EmployeeValidationUtil |
| `Identifiable` | Defines entities that have a unique identifier for persistence and lookup. Implemented by AbstractEntity and its subclasses (Employee, AttendanceRecord, LeaveRequest, PayrollResult). | AbstractEntity and subclasses |
| `ItTicket` | Domain model for IT support tickets. Child class extends AbstractEntity. | AbstractEntity, UserCredential (requestor) |
| `LeaveRequest` | Domain model for a leave request (OOP redesign - GEAR.HR). Child class extends AbstractEntity. Validates start date precedes end date and status is in allowed set. | AbstractEntity, Employee (by employeeId) |
| `PayrollData` | Domain model for payroll data per employee (OOP redesign - GEAR.HR). Implements Validatable only (no in-object id; does not extend AbstractEntity). Holds base salary, deductions, and allowances; used by PayrollProcessor. Amounts are stored at creation time (computed by PayrollUtils in service layer). | PayrollProcessor, PayrollUtils, Validatable |
| `PayrollResult` | DTO for payroll computation result (OOP redesign - GEAR.HR). Child class extends AbstractEntity. Carries computed breakdown for PayrollReport to format. | AbstractEntity, PayrollProcessor, PayrollReport |
| `UserCredential` | Domain model for user login credentials. Child class extends AbstractEntity. | AbstractEntity, CredentialValidationUtil, AuthenticationService |
| `Validatable` | Defines entities that can validate their own state. Implemented by AbstractEntity subclasses and by PayrollData. | AbstractEntity subclasses, PayrollData, PayPeriod |
| `PayPeriod` | **(Planned)** Domain model for a payroll month: month, year, status (draft, reviewed, finalized), finalizedBy, finalizedAt. Validates status transitions. Implements Validatable. One closed book per calendar month. | Validatable, PayrollProcessor, IPayPeriodRepository |
| `PayrollRunSnapshot` | **(Planned)** Frozen per-employee pay result for a pay period: periodId, employeeId, gross, deductions, allowances, net, hoursUsed. Historical reports use snapshots so live rate changes do not rewrite closed months. | PayPeriod, Employee (by id), IPayrollRunRepository, PayrollProcessor |
| `AuditLogEntry` | **(Planned)** Append-only audit record: userId, timestamp, optional targetEmployeeId, actionType (e.g. UPDATE_PAYROLL, FINALIZE_PERIOD). Supports HM3 accountability minimum. | IAuditLogRepository, PayrollProcessor |
| `DatabaseConnectionManager` | **(Planned)** Opens and closes JDBC connections to MySQL; supplies connections to repository classes. Centralizes connection configuration so repositories do not duplicate JDBC boilerplate. | MySQL JDBC driver, all *JdbcRepository classes |
| `AbstractJdbcRepository` | **(Planned)** Abstract template for JDBC repositories: shared query execution and row-mapping helpers; subclasses provide entity-specific SQL. Collaborator: DatabaseConnectionManager. Mirrors AbstractCsvListRepository pattern. | DatabaseConnectionManager, domain types (row mapping) |
| `AbstractCsvListRepository` | Abstract base for CSV repositories that persist a list of entities. **(Legacy — CSV fallback during migration.)** Subclasses provide file path, header, and line parsing/serialization; this class implements the common load/save file I/O. Concrete repos extend and implement the abstract hooks. | Concrete CSV repository subclasses |
| `EmployeeJdbcRepository` | **(Planned)** JDBC persistence for employees via MySQL. Implements IEmployeeRepository; load/save only. Collaborators: DatabaseConnectionManager, Employee. | DatabaseConnectionManager, IEmployeeRepository, Employee |
| `AttendanceJdbcRepository` | **(Planned)** JDBC persistence for attendance records. Implements IAttendanceRepository. Collaborators: DatabaseConnectionManager, AttendanceRecord. | DatabaseConnectionManager, IAttendanceRepository, AttendanceRecord |
| `LeaveRequestJdbcRepository` | **(Planned)** JDBC persistence for leave requests. Implements ILeaveRequestRepository. Collaborators: DatabaseConnectionManager, LeaveRequest. | DatabaseConnectionManager, ILeaveRequestRepository, LeaveRequest |
| `UserCredentialJdbcRepository` | **(Planned)** JDBC persistence for user credentials. Implements IUserCredentialRepository. Collaborators: DatabaseConnectionManager, UserCredential. | DatabaseConnectionManager, IUserCredentialRepository, UserCredential |
| `ItTicketJdbcRepository` | **(Planned)** JDBC persistence for IT tickets. Implements IItTicketRepository. Collaborators: DatabaseConnectionManager, ItTicket. | DatabaseConnectionManager, IItTicketRepository, ItTicket |
| `PayrollJdbcRepository` | **(Planned)** JDBC persistence for payroll settings (employeeId → PayrollData). Implements IPayrollRepository. Collaborators: DatabaseConnectionManager, PayrollData. | DatabaseConnectionManager, IPayrollRepository, PayrollData |
| `IPayPeriodRepository` | **(Planned)** Contract for pay period persistence: findById, save, listByStatus. Implementations swappable (JDBC). | PayPeriodJdbcRepository, PayPeriod, PayrollProcessor |
| `PayPeriodJdbcRepository` | **(Planned)** JDBC implementation of IPayPeriodRepository. Collaborators: DatabaseConnectionManager, PayPeriod. | DatabaseConnectionManager, IPayPeriodRepository, PayPeriod |
| `IPayrollRunRepository` | **(Planned)** Contract: saveAllForPeriod(periodId, snapshots), findByPeriod(periodId), findByPeriodAndEmployee. | PayrollRunJdbcRepository, PayrollRunSnapshot, PayrollProcessor |
| `PayrollRunJdbcRepository` | **(Planned)** JDBC implementation of IPayrollRunRepository. Collaborators: DatabaseConnectionManager, PayrollRunSnapshot. | DatabaseConnectionManager, IPayrollRunRepository, PayrollRunSnapshot |
| `IAuditLogRepository` | **(Planned)** Contract: append(entry); findByEmployee; findByPeriod for oversight reads. | AuditLogJdbcRepository, AuditLogEntry, PayrollProcessor |
| `AuditLogJdbcRepository` | **(Planned)** JDBC implementation of IAuditLogRepository. Append-only audit trail. Collaborators: DatabaseConnectionManager, AuditLogEntry. | DatabaseConnectionManager, IAuditLogRepository, AuditLogEntry |
| `AttendanceRepository` | CSV persistence for attendance records. Load/save only; no business logic. **(Legacy — CSV fallback.)** Implements IAttendanceRepository. Extends AbstractCsvListRepository for shared CSV load/save; provides AttendanceRecord-specific parse/serialize. Can be used as IAttendanceRepository by callers. | AbstractCsvListRepository, IAttendanceRepository, AttendanceRecord |
| `EmployeeRepository` | CSV persistence for employees (employees.csv). Load/save only; no business logic. **(Legacy — CSV fallback.)** Implements IEmployeeRepository. Extends AbstractCsvListRepository for shared CSV load/save; provides Employee-specific parse/serialize. Can be used as IEmployeeRepository by callers. | AbstractCsvListRepository, IEmployeeRepository, Employee |
| `IAttendanceRepository` | Contract for loading and saving attendance records. Implementations (e.g. CSV) can be swapped for testing or different storage. | AttendanceService, AttendanceJdbcRepository, AttendanceRepository |
| `IEmployeeRepository` | Contract for loading and saving employee data. Implementations (e.g. CSV) can be swapped for testing or different storage. | EmployeeService, EmployeeJdbcRepository, EmployeeRepository |
| `IItTicketRepository` | Contract for loading and saving IT support tickets. Implementations (e.g. CSV) can be swapped for testing or different storage. | ItTicketService, ItTicketJdbcRepository, ItTicketRepository |
| `ILeaveRequestRepository` | Contract for loading and saving leave requests. Implementations (e.g. CSV) can be swapped for testing or different storage. | LeaveService, LeaveRequestJdbcRepository, LeaveRequestRepository |
| `IPayrollRepository` | Contract for loading and saving payroll data (employeeId -> PayrollData). Implementations (e.g. CSV) can be swapped for testing or different storage. | PayrollProcessor, PayrollJdbcRepository, PayrollRepository |
| `ItTicketRepository` | CSV persistence for IT support tickets. Load/save only; no business logic. **(Legacy — CSV fallback.)** Implements IItTicketRepository. Extends AbstractCsvListRepository for shared CSV load/save; provides ItTicket-specific parse/serialize. Can be used as IItTicketRepository by callers. | AbstractCsvListRepository, IItTicketRepository, ItTicket |
| `IUserCredentialRepository` | Contract for loading and saving user credentials. Implementations (e.g. CSV) can be swapped for testing or different storage. | UserCredentialService, UserCredentialJdbcRepository, UserCredentialRepository |
| `LeaveRequestRepository` | CSV persistence for leave requests. Load/save only; no business logic. **(Legacy — CSV fallback.)** Implements ILeaveRequestRepository. Extends AbstractCsvListRepository for shared CSV load/save; provides LeaveRequest-specific parse/serialize. Can be used as ILeaveRequestRepository by callers. | AbstractCsvListRepository, ILeaveRequestRepository, LeaveRequest |
| `PayrollRepository` | CSV persistence for payroll data (employeeId -> PayrollData). Load/save only. **(Legacy — CSV fallback.)** Expects map values to already include withholding tax when saving. Implements IPayrollRepository. Can be used as IPayrollRepository by callers. | IPayrollRepository, PayrollData |
| `UserCredentialRepository` | CSV persistence for user credentials. Load/save only; no business logic. **(Legacy — CSV fallback.)** Implements IUserCredentialRepository. Extends AbstractCsvListRepository for shared CSV load/save; provides UserCredential-specific parse/serialize. Can be used as IUserCredentialRepository by callers. | AbstractCsvListRepository, IUserCredentialRepository, UserCredential |
| `ApplicationContext` | Composition root: wires **JDBC** repository implementations for MySQL at startup (target); optional flag/factory to use CSV repos during migration. Creates services; passed into UI (DI). Stores and exposes interface types so callers depend on abstractions. Collaborators: all I*Repository interfaces, *JdbcRepository classes, services. | All I*Repository implementations, all services, UI modules (via DI) |
| `AttendanceService` | Manages attendance records; persistence delegated to repository (OOP redesign - GEAR.HR). Implements IAttendanceService. Can be used as IAttendanceService by callers; depends on IAttendanceRepository. | IAttendanceRepository, AttendanceRecord, PayrollProcessor (hours) |
| `AuthContext` | Immutable authentication context value object. | AuthenticationService, User |
| `AuthenticationService` | Concrete authentication implementation used via IAuthenticationService. Loads and validates credentials independent of UI. | IUserCredentialRepository, EmployeeService, User |
| `EmployeeService` | Manages employee data; persistence delegated to repository (OOP redesign - GEAR.HR). Implements IEmployeeService. Can be used as IEmployeeService by callers; depends on IEmployeeRepository. | IEmployeeRepository, IUserCredentialService, EmployeeValidationUtil, Employee |
| `IAttendanceService` | Contract for attendance record management: load and CRUD. Implementations can be swapped for testing or different backends. | AttendanceService, AttendanceScreen |
| `IAuthenticationService` | Authentication contract used by callers. | AuthenticationService, User, UserCredentialService |
| `IEmployeeService` | Contract for employee management: load/save and CRUD. Implementations can be swapped for testing or different backends. | EmployeeService, EmployeeProfile, AttendanceScreen |
| `IItTicketService` | Contract for IT ticket use cases: reload, list, create, update status, delete. | ItTicketService, UserCredentialManagementScreen |
| `ILeaveService` | Contract for leave request management: load and CRUD. Implementations can be swapped for testing or different backends. | LeaveService, LeaveManagementScreen |
| `ItTicketService` | Service implementation for IT ticket use cases. Maintains in-memory ticket list and persists via repository abstraction. | IItTicketRepository, IAuthenticationService, ItTicket |
| `IUserCredentialService` | Contract for credential administration: reload, query, and update accounts. | UserCredentialService, UserCredentialManagementScreen, EmployeeService |
| `LeaveService` | Manages leave requests; persistence delegated to repository (OOP redesign - GEAR.HR). Implements ILeaveService. Can be used as ILeaveService by callers; depends on ILeaveRequestRepository. | ILeaveRequestRepository, LeaveRequest, EmployeeService |
| `PayrollProcessor` | Computes employee compensation; orchestrates payroll persistence and HM3 use cases. **(Planned additions:** batch run for all employees per period, load snapshots, finalize period, append audit after successful saves.) Depends on IPayrollRepository, IAttendanceService, IPayPeriodRepository, IPayrollRunRepository, IAuditLogRepository. | IPayrollRepository, IAttendanceService, IPayPeriodRepository, IPayrollRunRepository, IAuditLogRepository, PayrollUtils, PayrollData, Employee |
| `PayrollReport` | Formats payroll results for display. **(Planned additions:** payroll register and deduction summary from PayrollRunSnapshot lists.) Separates presentation text from PayrollProcessor computation. | PayrollResult, PayrollRunSnapshot, EmployeeProfile (display) |
| `RoleGroup` | Role-based access: classifies user role string into HR, Payroll, IT/Admin, or Normal Employee group. Used by Main and screens to show the correct homepage and enforce access restrictions. Enum + factory #fromRole(String) classify roles without string subclassing. | Main, ModuleScreen implementations, AuthenticationService |
| `UserCredentialService` | Service implementation for credential administration. Owns loaded credential collection and persistence lifecycle. | IUserCredentialRepository, IAuthenticationService, UserCredential |
| `AttendanceScreen` | Attendance UI screen; services injected (DI). Named AttendanceScreen to avoid confusion with domain/service. Implements ModuleScreen. Extends BaseModuleScreen for shared colors and helpers. INSTANCE can be used as ModuleScreen by Main. | ApplicationContext, IAttendanceService, IEmployeeService, ModuleScreen, BaseModuleScreen |
| `BaseModuleScreen` | Abstract base for module screens; provides shared colors and layout helpers. Subclasses use protected colors and createFrame/createHeaderPanel/createFooterPanel to avoid duplication. Content is built by each subclass. | ModuleScreen subclasses |
| `EmployeeProfile` | EmployeeProfile UI; employee and payroll data delegated to services (OOP redesign). Implements ModuleScreen. Extends BaseModuleScreen for shared colors and helpers. INSTANCE can be used as ModuleScreen by Main. | ApplicationContext, IEmployeeService, IAttendanceService, PayrollProcessor, ModuleScreen |
| `InputFilters` | UI-layer input restrictions for employee forms. Keeps the domain/persistence layer responsible for final validation via EmployeeValidationUtil. Inner RegexOnlyDocumentFilter extends DocumentFilter to restrict input. Non-instantiable facade; applies filters via static methods. | EmployeeProfile (forms), EmployeeValidationUtil |
| `InputFilters.RegexOnlyDocumentFilter` | Private DocumentFilter that allows only text matching a configured regex pattern when editing Swing text fields. | InputFilters |
| `LeaveManagementScreen` | Leave management UI; view, create, and update leave request status. Services injected (DI). Implements ModuleScreen. Extends BaseModuleScreen for shared colors and helpers. INSTANCE can be used as ModuleScreen by Main. | ApplicationContext, ILeaveService, IEmployeeService, ModuleScreen |
| `Main` | Main class for the Employee Management System. Central dashboard witnih navigation to modules. Opens module screens via ModuleScreen interface so any screen can be shown uniformly. | ApplicationContext, ModuleScreen, RoleGroup, User |
| `ModuleScreen` | Contract for module screens that can be shown with a single polymorphic call. Enables Main to open any screen without knowing the concrete class. | Main, concrete screen classes |
| `SplashScreen` | SplashScreen provides an animated loading screen before the main application loads. Bootstrap UI only; no domain or service dependencies. | Main (bootstrap only) |
| `User` | User class handles login UI; authentication delegated to AuthenticationService (OOP redesign). Delegates authentication to service.IAuthenticationService via ApplicationContext. | ApplicationContext, IAuthenticationService, Main |
| `UserCredentialManagementScreen` | Admin UI for credentials and IT tickets; services from ApplicationContext (DI). Implements ModuleScreen. Extends BaseModuleScreen for shared colors and helpers. INSTANCE can be used as ModuleScreen by Main. | ApplicationContext, IUserCredentialService, IItTicketService, ModuleScreen |
| `CredentialValidationUtil` | Utility class; validates credentials via static helpers and private constructor. | UserCredential, UserCredentialService |
| `EmployeeValidationUtil` | Format validation for employee profile fields (Employee Profile Validation Plan). Each method returns null if valid, or an error message describing the parameter and required format. Utility class with private constructor; validation rules encapsulated as patterns. | Employee, EmployeeService |
| `PayrollUtils` | Utility class: private constructor prevents instantiation; static payroll calculation helpers. | PayrollData, PayrollProcessor |
| `TableColumnSortUtil` | Installs header-click sorting on a JTable with at most one sort column at a time. Numeric columns compare parsed numbers; other columns compare case-insensitive strings. Unparseable numeric cells sort last (treated as positive infinity when ascending). Hides TableRowSorter setup behind a single install method. Non-instantiable utility type. | UI JTable screens |
| `ApplicationContext.ItTicketRepositoryFallback` | Anonymous no-op IItTicketRepository used when ItTicketRepository cannot be loaded. | ApplicationContext, IItTicketRepository |
...
## Planned additions (JDBC and HM3)

Methods below are **not yet in the codebase**; they document the target design from HM4 and our planning logs.

| Class | Method Name | Purpose | Visibility | Parameters | Return type |
|-------|-------------|---------|------------|------------|-------------|
| `DatabaseConnectionManager` | `getConnection` | Returns a JDBC Connection to MySQL. | public | none | `Connection` |
| `DatabaseConnectionManager` | `closeConnection` | Closes the given connection safely. | public | Connection conn | `void` |
| `AbstractJdbcRepository` | `executeQuery` | Runs a prepared SELECT and returns a ResultSet (subclasses use for load). | protected | String sql, Object... params | `ResultSet` |
| `AbstractJdbcRepository` | `executeUpdate` | Runs INSERT/UPDATE/DELETE. | protected | String sql, Object... params | `int` |
| `EmployeeJdbcRepository` | `load` | Loads all employees from MySQL. Implements IEmployeeRepository. | public | none | `List<Employee>` |
| `EmployeeJdbcRepository` | `save` | Saves employee list to MySQL. | public | List<Employee> employees | `void` |
| `EmployeeJdbcRepository` | `findById` | Finds one employee by employee number. | public | String employeeNumber | `Employee` |
| `AttendanceJdbcRepository` | `load` | Loads all attendance records. | public | none | `List<AttendanceRecord>` |
| `AttendanceJdbcRepository` | `save` | Saves attendance list. | public | List<AttendanceRecord> records | `void` |
| `LeaveRequestJdbcRepository` | `load` | Loads all leave requests. | public | none | `List<LeaveRequest>` |
| `LeaveRequestJdbcRepository` | `save` | Saves leave request list. | public | List<LeaveRequest> requests | `void` |
| `UserCredentialJdbcRepository` | `load` | Loads all credentials. | public | none | `List<UserCredential>` |
| `UserCredentialJdbcRepository` | `save` | Saves credential list. | public | List<UserCredential> credentials | `void` |
| `ItTicketJdbcRepository` | `load` | Loads all IT tickets. | public | none | `List<ItTicket>` |
| `ItTicketJdbcRepository` | `save` | Saves ticket list. | public | List<ItTicket> tickets | `void` |
| `PayrollJdbcRepository` | `load` | Loads payroll map from MySQL. | public | none | `Map<String, PayrollData>` |
| `PayrollJdbcRepository` | `save` | Saves payroll map. | public | Map<String, PayrollData> data | `void` |
| `PayPeriod` | `PayPeriod` | Constructs pay period with month, year, status. | public | int month, int year, String status | `—` |
| `PayPeriod` | `getPeriodId` | Returns business key (e.g. yyyy-MM). | public | none | `String` |
| `PayPeriod` | `getMonth` | Returns month. | public | none | `int` |
| `PayPeriod` | `getYear` | Returns year. | public | none | `int` |
| `PayPeriod` | `getStatus` | Returns status (draft, reviewed, finalized). | public | none | `String` |
| `PayPeriod` | `getFinalizedBy` | Returns user id who finalized. | public | none | `String` |
| `PayPeriod` | `getFinalizedAt` | Returns finalize timestamp. | public | none | `LocalDateTime` |
| `PayPeriod` | `isValid` | Validates period state. | public | none | `boolean` |
| `PayPeriod` | `canTransitionTo` | Returns whether status change is allowed. | public | String newStatus | `boolean` |
| `PayrollRunSnapshot` | `PayrollRunSnapshot` | Constructs frozen pay result for one employee in a period. | public | String periodId, String employeeId, double gross, double net, double hoursUsed, ... | `—` |
| `PayrollRunSnapshot` | `getPeriodId` | Returns period id. | public | none | `String` |
| `PayrollRunSnapshot` | `getEmployeeId` | Returns employee id. | public | none | `String` |
| `PayrollRunSnapshot` | `getGrossPay` | Returns gross pay at run time. | public | none | `double` |
| `PayrollRunSnapshot` | `getNetPay` | Returns net pay at run time. | public | none | `double` |
| `PayrollRunSnapshot` | `getHoursUsed` | Returns attendance hours used. | public | none | `double` |
| `AuditLogEntry` | `AuditLogEntry` | Constructs audit row. | public | String userId, LocalDateTime timestamp, String actionType, String targetEmployeeId | `—` |
| `AuditLogEntry` | `getUserId` | Returns acting user. | public | none | `String` |
| `AuditLogEntry` | `getTimestamp` | Returns when action occurred. | public | none | `LocalDateTime` |
| `AuditLogEntry` | `getActionType` | Returns action type enum/string. | public | none | `String` |
| `AuditLogEntry` | `getTargetEmployeeId` | Returns affected employee if any. | public | none | `String` |
| `IPayPeriodRepository` | `findById` | Loads pay period by id. | package | String periodId | `PayPeriod` |
| `IPayPeriodRepository` | `save` | Inserts or updates pay period. | package | PayPeriod period | `void` |
| `IPayPeriodRepository` | `listByStatus` | Lists periods matching status. | package | String status | `List<PayPeriod>` |
| `PayPeriodJdbcRepository` | `findById` | JDBC implementation. | public | String periodId | `PayPeriod` |
| `PayPeriodJdbcRepository` | `save` | JDBC implementation. | public | PayPeriod period | `void` |
| `PayPeriodJdbcRepository` | `listByStatus` | JDBC implementation. | public | String status | `List<PayPeriod>` |
| `IPayrollRunRepository` | `saveAllForPeriod` | Persists all snapshots for a period (batch insert). | package | String periodId, List<PayrollRunSnapshot> snapshots | `void` |
| `IPayrollRunRepository` | `findByPeriod` | Loads all snapshots for a period. | package | String periodId | `List<PayrollRunSnapshot>` |
| `IPayrollRunRepository` | `findByPeriodAndEmployee` | Loads one employee snapshot for a period. | package | String periodId, String employeeId | `PayrollRunSnapshot` |
| `PayrollRunJdbcRepository` | `saveAllForPeriod` | JDBC implementation. | public | String periodId, List<PayrollRunSnapshot> snapshots | `void` |
| `PayrollRunJdbcRepository` | `findByPeriod` | JDBC implementation. | public | String periodId | `List<PayrollRunSnapshot>` |
| `PayrollRunJdbcRepository` | `findByPeriodAndEmployee` | JDBC implementation. | public | String periodId, String employeeId | `PayrollRunSnapshot` |
| `IAuditLogRepository` | `append` | Appends one audit entry (insert only). | package | AuditLogEntry entry | `void` |
| `IAuditLogRepository` | `findByEmployee` | Reads audit history for an employee. | package | String employeeId | `List<AuditLogEntry>` |
| `IAuditLogRepository` | `findByPeriod` | Reads audit entries tied to a period. | package | String periodId | `List<AuditLogEntry>` |
| `AuditLogJdbcRepository` | `append` | JDBC implementation. | public | AuditLogEntry entry | `void` |
| `AuditLogJdbcRepository` | `findByEmployee` | JDBC implementation. | public | String employeeId | `List<AuditLogEntry>` |
| `AuditLogJdbcRepository` | `findByPeriod` | JDBC implementation. | public | String periodId | `List<AuditLogEntry>` |
| `PayrollProcessor` | `PayrollProcessor` | **(Planned)** Extended constructor injecting period, run, and audit repositories. | public | IPayrollRepository, IAttendanceService, IPayPeriodRepository, IPayrollRunRepository, IAuditLogRepository | `—` |
| `PayrollProcessor` | `getPayrollForEmployee` | Returns pay for employee and period; uses snapshot if finalized else live compute. | public | String employeeId, String periodId | `PayrollResult` |
| `PayrollProcessor` | `runPayrollForPeriod` | Batch-computes and saves snapshots for all employees. | public | String periodId | `List<PayrollRunSnapshot>` |
| `PayrollProcessor` | `loadSnapshotsForPeriod` | Loads stored snapshots for a period. | public | String periodId | `List<PayrollRunSnapshot>` |
| `PayrollProcessor` | `finalizePeriod` | Sets period to finalized, records user, appends audit. | public | String periodId, String userId | `void` |
| `PayrollProcessor` | `updatePayrollData` | **(Planned change)** After successful save, appends UPDATE_PAYROLL audit entry. | public | String employeeId, PayrollData data, String userId | `void` |
| `PayrollReport` | `formatPayrollRegister` | Formats company-wide register from snapshots. | public static | List<PayrollRunSnapshot> snapshots, PayPeriod period | `String` |
| `PayrollReport` | `formatDeductionSummary` | Formats statutory deduction totals for a period. | public static | String periodId, List<PayrollRunSnapshot> snapshots | `String` |
| `ApplicationContext` | `ApplicationContext` | **(Planned)** Constructor wires JDBC repos and extended PayrollProcessor. | public | none | `—` |
