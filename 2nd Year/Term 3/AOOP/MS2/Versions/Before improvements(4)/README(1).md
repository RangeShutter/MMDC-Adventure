...
### Storage backend switch (CSV → JDBC transition)

The app migrated from CSV-backed persistence to **JDBC repositories**. Both backends still exist behind the `I*Repository` interfaces, and `storage.mode` in `database.properties` selects the one wired by `[src/service/ApplicationContext.java](src/service/ApplicationContext.java)`:

- `jdbc` (default) — MySQL via the `*JdbcRepository` classes (e.g. `EmployeeJdbcRepository`, `PayrollJdbcRepository`, `AttendanceJdbcRepository`)
- `csv` — legacy CSV repositories under `csv/`

Service code (e.g. `PayrollProcessor`, `EmployeeService`) is unchanged across backends because it depends on the repository interfaces, not on CSV or JDBC directly.
...
These CSVs are only used when `storage.mode=csv`. In the default `jdbc` mode, data is read from and written to the MySQL `gear.hr` tables instead, and `sql/schema.sql` defines the equivalent tables. If a backing source (CSV file or DB table) is empty/missing, the app usually starts with empty data for that module.
...
1. Set the **role** to an exact string from the lists above — in the `user_credentials` table (JDBC mode) or in `csv/user_credentials.csv` (CSV mode).
